...
import { IndicesView } from './indices-view.js';
...
function initStocks() {
    ...
    for (const [code, team] of Object.entries(TEAM_REGISTRY)) {
        stocks[code] = {
            ...
            changePct: 0,
            volume: '0.0M',
            marketCap: team.basePrice.toFixed(2) + 'B',
            momentum: 50,
            confidence: 'Medium',
            volatility: 'Medium',
...
const Components = {
    ...
    matchCard(match) {
        ...
        const liveIndicator = status === 'LIVE' ? '<span class=\"live-badge\" style=\"margin-left:8px;padding:4px 10px;font-size:10px;\"><span class=\"live-dot\"></span>LIVE</span>' : '';
...
function startLiveMatch() {
    ...
    const interval = setInterval(() => {
        ...
        if (match.minute >= 90) {
            ...
            match.time = match.minute + ' MIN';
            if (Math.random() < 0.15) {
                if (Math.random() < 0.5) match.goalsA++;
                else match.goalsB++;
            }
        }
        updateLiveMatchDisplay();
...
function finalizeMatch(match) {
    ...
    updateIndices();
    if (App.currentView === 'overview') App.renderView('overview');
    if (App.currentView === 'matches') App.renderView('matches');
}
...
function updateStockPrice(symbol, changePct, match) {
    ...
    const recent = stock.sparkline.slice(-5);
    const variance = recent.reduce((sum, v, i, arr) => i > 0 ? sum + Math.abs(v - arr[i-1]) : sum, 0) / 4;
    ...
    stock.marketCap = stock.price.toFixed(2) + 'B';
...
function updateIndices() {
    ...
    const mom = Object.values(STOCKS).reduce((a, s) => a + s.momentum, 0) / prices.length;
    LIVE_DATA.indices[0].value = Math.round(avg * 20 * 100) / 100;
    LIVE_DATA.indices[0].change = Math.round((avg * 20 - 2847.32) * 100) / 100;
    LIVE_DATA.indices[0].changePct = Math.round((LIVE_DATA.indices[0].change / 2847.32) * 10000) / 100;
    LIVE_DATA.indices[3].value = Math.round(mom * 100) / 100;
    LIVE_DATA.indices[3].change = Math.round((mom - 67.21) * 100) / 100;
    LIVE_DATA.indices[3].changePct = Math.round((LIVE_DATA.indices[3].change / 67.21) * 10000) / 100;
}
...
function renderPortfolioSummary() {
    const prices = Object.fromEntries(Object.entries(STOCKS).map(([k, v]) => [k, v.price]));
    const val = PORTFOLIO.getValue(prices);
    const pnl = val.total - 100000;
    const pnlPct = (pnl / 100000) * 100;
    return `<div style=\"display:flex;flex-direction:column;gap:var(--space-3);\"><div style=\"display:flex;justify-content:space-between;\"><span style=\"color:var(--text-secondary);\">Cash</span><span class=\"text-mono\" style=\"font-weight:600;\">$${val.cash.toLocaleString()}</span></div><div style=\"display:flex;justify-content:space-between;\"><span style=\"color:var(--text-secondary);\">Stocks</span><span class=\"text-mono\" style=\"font-weight:600;\">$${val.stockValue.toLocaleString()}</span></div><div style=\"height:1px;background:var(--border-subtle);margin:var(--space-2) 0;\"></div><div style=\"display:flex;justify-content:space-between;\"><span style=\"font-weight:600;\">Total</span><span class=\"text-mono\" style=\"font-weight:700;font-size:18px;\">$${val.total.toLocaleString()}</span></div><div style=\"display:flex;justify-content:space-between;\"><span style=\"color:var(--text-secondary);\">P&L</span><span class=\"text-mono ${pnl >= 0 ? 'text-up' : 'text-down'}\" style=\"font-weight:600;\">${pnl >= 0 ? '+' : ''}$${pnl.toLocaleString()} (${pnl >= 0 ? '+' : ''}${pnlPct.toFixed(2)}%)</span></div><div style=\"margin-top:var(--space-3);\"><button onclick=\"window.resetPortfolio()\" style=\"width:100%;padding:10px;background:var(--bg-elevated);color:var(--text-secondary);border:1px solid var(--border-subtle);border-radius:var(--radius-sm);font-family:var(--font-mono);font-size:12px;cursor:pointer;\">RESET PORTFOLIO</button></div></div>`;
}
...
Views['indices'] = function() { return IndicesView.render(STOCKS); };
Views['watchlist'] = function() { return Watchlist.render(STOCKS); };
Views['correlations'] = function() { return Correlations.render(STOCKS); };
